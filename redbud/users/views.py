...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "manager":
            ...
            training_ids = Training.objects.filter(assigned_trainer=user).values_list(
                "id", flat=True
            )
            employee_ids = Training.objects.filter(id__in=training_ids).values_list(
                "employees", flat=True
            )
...
class TrainingViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        request={"application/json": {"example": {"employee_ids": [1, 2, 3]}}},
    ...
    @extend_schema(
        ...
        request={"application/json": {"example": {"trainer_id": 1}}},
        responses={200: TrainingSerializer},
        tags=["Trainings"],
    )
    @action(detail=True, methods=["post"], permission_classes=[IsManager])
